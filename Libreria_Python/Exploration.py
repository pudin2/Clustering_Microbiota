...
from DB_Scan import prepare_data_for_dbscan


class HTMLArtifact:
    artifact_kind = "html"

    def __init__(self, title, html, filename=None):
    ...
    if x_col and y_col:
        for col in [x_col, y_col]:
            if col not in df.columns:
                raise KeyError(f"La columna '{col}' no existe en '{df_name}'")
        plot_df = df[[x_col, y_col] + ([hue_col] if hue_col and hue_col in df.columns else [])].copy()
        ...
        if hue_col and hue_col in plot_df.columns:
            sns.scatterplot(data=plot_df, x=x_col, y=y_col, hue=hue_col, alpha=0.75)
        else:
            sns.scatterplot(data=plot_df, x=x_col, y=y_col, alpha=0.75)
        plt.title(f"Variables conjuntas: {x_col} vs {y_col}")
        plt.grid(True, alpha=0.25)
        plt.tight_layout()
        plt.show()

        output["joint_plot_data"] = plot_df

    violin_cols = _as_list(violin_cols)
    if violin_cols:
        missing = [col for col in violin_cols if col not in df.columns]
        if missing:
        ...
        for col in violin_cols:
            plot_df = df[[col] + ([group_col] if group_col else [])].copy()
            plot_df[col] = pd.to_numeric(plot_df[col], errors="coerce")
            plot_df = plot_df.dropna(subset=[col])

            plt.figure(figsize=(8, 5))
            if group_col:
                sns.violinplot(data=plot_df, x=group_col, y=col, inner="quartile", cut=0)
                plt.xticks(rotation=35, ha="right")
            else:
                sns.violinplot(y=plot_df[col], inner="quartile", cut=0)
            plt.title(f"Grafico de violin: {col}")
            plt.tight_layout()
            plt.show()

            if group_col:
                grouped = plot_df.groupby(group_col)[col]
                ...
                summary_rows.append({
                    "variable": col,
                    "group": "all",
                    "n": int(plot_df[col].notna().sum()),
                    "median": float(plot_df[col].median()),
                    "mean": float(plot_df[col].mean()),
                })

        output["violin_summary"] = pd.DataFrame(summary_rows)

    if rank_abundance:
        abundance_cols = _as_list(abundance_cols)
        if abundance_cols is None:
            abundance_cols = [
                col for col in df.select_dtypes(include=[np.number]).columns
                if col != abundance_id_col
            ]
        missing = [col for col in abundance_cols if col not in df.columns]
        if missing:
            raise KeyError(f"Estas columnas de abundancia no existen: {missing}")

        abundance = df[abundance_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
        ...
        rank_df = pd.DataFrame({
            "rank": np.arange(1, len(totals) + 1),
            "feature": totals.index,
            "abundance": totals.values,
            "relative_abundance": totals.values / totals.sum(),
            "log10_abundance": np.log10(totals.values),
        })

        plt.figure(figsize=(9, 5))
        plt.plot(rank_df["rank"], rank_df["abundance"], marker=".", linewidth=1)
        if log_scale:
            plt.yscale("log")
        plt.xlabel("Rank")
        plt.ylabel("Abundancia total")
        plt.title("Rank-abundancia")
        plt.grid(True, alpha=0.25)
        plt.tight_layout()
        plt.show()

        output["rank_abundance"] = rank_df
        ...
        if px is not None:
            fig = px.line(
                ...
                log_y=bool(log_scale),
                title="Rank-abundancia interactiva",
            )
            html = fig.to_html(full_html=True, include_plotlyjs=True)
            output["rank_abundance_interactive"] = HTMLArtifact(
                "Rank-abundancia interactiva",
                html,
                filename="rank_abundance_interactive.html",
            )

    if verbose:
        pieces = [key for key in output.keys()]
