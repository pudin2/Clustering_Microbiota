...
class OpenAssistantEngine:
    ...
    def answer(self, question, selected_dataset=None, use_ollama=False, model="qwen2.5:3b"):
        ...
        if use_ollama:
            try:
                response.text = self._ollama_answer(question, info, response, model=model)
            except Exception as exc:
                response.warnings.append(f"No pude usar Ollama local: {exc}")
                response.text += (
                    "\n\nNota: intente usar Ollama local, pero no respondio. "
                    "Deje la recomendacion local basada en reglas."
                )
    ...
    def _ollama_answer(self, question, info, local_response, model):
        prompt = {
            "role": "Eres un asistente estadistico para una app de microbiota. Responde en español claro, amable y accionable.",
            "question": question,
            "dataset_context": self._compact_info(info),
            ...
            "instruction": (
                "Explica que prueba o preprocesamiento conviene, que parametros revisar y que riesgos mirar. "
                "No inventes columnas fuera del contexto."
            ),
        }
        ...
        if not text:
            raise RuntimeError("Ollama respondio sin texto.")
        return text
